...
public class GraphView extends JPanel implements MouseInputListener{
	...
	/*
	 * finds a match if a point has mapped coordinates close to a point in the graph
	 */
	public ColoredPoint findMatch(ColoredPoint newPoint){
		...
		try{
			...
			while(itr.hasNext()){
				foundPoint = itr.next();
				if((Math.abs(foundPoint.getMappedX() - newPoint.getMappedX()) < 10) && (Math.abs(foundPoint.getMappedY() - newPoint.getMappedY()) < 10)){
					match = foundPoint;
				}
			}
		}
		catch(NoSuchElementException ex){
			JOptionPane.showMessageDialog(null, "You have no data");
		}
		return match;
	}
	...
	/*
	 * displays if a mouse is in the graph frame
	 */
	@Override
	...
	/*
	 * displays the toString() of a point if point is selected
	 */
	@Override
	...
	/*
	 * mouse released will change the text back to No point selected.
	 */
	@Override
